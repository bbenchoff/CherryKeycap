...
// decoration
obj = "HelmetSub.stl";            // decoration object
obj_pos = [0,0,.2];        // decoration offset
obj_scale = [.12,.12,.15]; // decoration scale
...
module shape()
{
    hull()
    {
        ...
        rotate([head_tilt,0,0]) translate([0,0,6.5]) scale([.113,.113,.14]) linear_extrude(height=					2)top();
    }
}
